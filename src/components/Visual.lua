...
---@param array table
---@public
function RageUI.Popup(msg)
    AddTextEntry('NotificationSimple', msg)
    SetNotificationTextEntry("NotificationSimple")
...
function RageUI.PopupChar(array)
    ...
    if (array.picture ~= nil) then
        ...
        if (array.iconTypes == 1) or (array.iconTypes == 2) or (array.iconTypes == 3) or (array.iconTypes == 7) or (array.iconTypes == 8) or (array.iconTypes == 9) then
            SetNotificationMessage('CHAR_ALL_PLAYEns_CONF', 'CHAR_ALL_PLAYEns_CONF', true, array.iconTypes, array.sender, array.title)
        else
            SetNotificationMessage('CHAR_ALL_PLAYEns_CONF', 'CHAR_ALL_PLAYEns_CONF', true, 4, array.sender, array.title)
        end
    end
